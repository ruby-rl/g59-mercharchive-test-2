...
function buildCollectionSection(categoryKey, folderName) {
  ...
  const shortTitle = info.shortTitle || title;
...
function buildSidebarHtml(sections) {
  if (!sections.length) return "";
  const items = sections
    .map((s) => `<a href="#${s.id}" data-target="${s.id}">${escapeHtml(s.navLabel)}</a>`)
    .join("\n      ");
...
function buildCategoryPage(category, categories) {
  ...
  const { sections, blockHtml } = buildCategoryBlock(category);
